...
const POINTS = [
  ...
  // lon은 실제 좌표(129.410719158274)에서 동쪽(바다 방향)으로 약 0.01도(≈0.9km)
  // 조정한 값. 원래 좌표는 Open-Meteo Marine API 격자점이 elevation=3(육지)으로
  // 분류돼 파고 예보가 전부 null이었음(직접 호출로 확인, 2026-09-14). 이 값은
  // 같은 방식으로 forecast_days=8 전체(192시간) wave_height/swell_wave_height가
  // null 없이 나오는 것까지 실제 API 응답으로 확인한 값이다.
  { id: "goraebul", area: "영덕", name: "영덕 고래불", lat: 36.5986693176069, lon: 129.420719158274, khoaObsCode: "TW_0095", khoaStationName: "고래불해수욕장" },
